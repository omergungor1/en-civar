...
export default function BusinessActionButtons({ business }) {
    const handlePhoneClick = (e) => {
        e.preventDefault();
        if (typeof window !== 'undefined' && window.gtag_report_conversion) {
            window.gtag_report_conversion(`tel:${business.phone.trim()}`);
        } else {
            window.location.href = `tel:${business.phone.trim()}`;
        }
    };
